...
class WaterBirdsDataset(Dataset):
    def __init__(self, basedir, split="train", core_noise:int=0, transform=None ):
        try:
            split_i = ["train", "val", "test"].index(split)
        except ValueError:
            raise(f"Unknown split {split}")
        assert core_noise in [0,2,4,6,8,10], f"Invalid core noise value: {core_noise}"
        metadata_df = pd.read_csv(os.path.join(basedir, f"metadata_{core_noise}.csv"))
        # print(len(metadata_df))
        self.metadata_df = metadata_df[metadata_df["split"] == split_i]
        print("loading split {}, len = {}, core_noise = {:.2f}%, spurious_noise = {:.2f}%".format(
            split, 
            len(self.metadata_df),
            self.metadata_df.loc[
                    self.metadata_df["gth"] == self.metadata_df["y"]
                ].shape[0] / self.metadata_df.shape[0] * 100,
            self.metadata_df.loc[
                    self.metadata_df["place"] == self.metadata_df["y"]
                ].shape[0] / self.metadata_df.shape[0] * 100,
            ))
